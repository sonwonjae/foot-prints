...
import { parseValueToMarkdownList, parseMarkdownListToValue } from "@/utils/markdown";
import { toggleStyledText } from "@/utils/markdown/styledText";

import { ChangeSelectionRange, GapType, ReplaceSelectionMarkdown } from "./types";

import type { KeyboardEvent as ReactKeyboardEvent } from "react";

export const getMarkdownIndexMap = (e: ReactKeyboardEvent<HTMLTextAreaElement>) => {
...
export const getSelectionType = ({
    ...
    switch (true) {
        ...
        case index ===
            startIndexMap.indexInMarkdownList:
            return 'first-line';
...
export const parseMarkdownWithSelection = ({
    type,
    markdown,
    startIndexMap,
    endIndexMap,
...
export const calculateSelection = ({
    ...
    if (newMarkdown === null) {
        return {
            ...
            nextSelectionEnd:
                nextSelectionEnd - markdown.length - 1,
        };
    }
    ...
    if (type === 'same-selection' || type === 'same-line') {
        if (nextSelectionSameGapType === 'include') {
            return {
                nextSelectionStart: nextSelectionStart,
                nextSelectionEnd:
                    nextSelectionEnd + gap,
            };
        }
        if (nextSelectionSameGapType === 'exclude') {
            return {
                nextSelectionStart:
                    nextSelectionStart + gap,
                nextSelectionEnd:
                    nextSelectionEnd + gap,
            };
        }
...
export const judgeFinalChangeValueParam = ({
    moveTo,
    changeValueParam,
}: {
    moveTo: Parameters<ChangeSelectionRange>[0]['moveTo'];
